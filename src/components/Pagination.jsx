...
const Pagination = ({ page, setPageNo, pageNo }) => {
  ...
  useEffect(() => {
    ...
    } else if (pageNo > 4 && pageNo < numberOfPages.length - 2) {
      // from 5 to 8 -> (10 - 2)
      const sliced1 = numberOfPages.slice(pageNo - 2, pageNo); // sliced1 (5-2, 5) -> [4,5]
      const sliced2 = numberOfPages.slice(pageNo, pageNo + 2); // sliced2 (5, 5+2) -> [6,7]
      tempNumberOfPages = [
        1,
        dotsLeft,
        ...sliced1,
    ...
    // eslint-disable-next-line
  }, [pageNo, numberOfPages, currentPage, numberOfPages.length]);

  return (
    <>
      <div className='flex mt-4 justify-center pb-10 items-center gap-4 px-10'>
        {/* next and previoius n */}
        {/* Previous Button */}
        <button
          className={
            'w-8 h-8 rounded grid place-content-center bg-gray-100 text-charlieGray  '
          }
          onClick={() =>
        ...
        <div className='flex items-center gap-2'>
          {arrOfCurrButtons.map((item, index) => (
            <div
              key={index}
              className={`w-8 h-8 grid rounded place-content-center text-sm  cursor-pointer + ${
                pageNo === item
        ...
        <button
          className={
            'w-8 h-8 grid place-content-center rounded bg-gray-100 text-charlieGray '
          }
          onClick={() =>
            setPageNo((prev) =>
